...
@app.route("/test")
def test():
	return render_template('messreports/dailystudent_messreport.html')
...
def studentAddBMIPage(className,rollNumber):
    ...
    if request.method=='POST':
        ...
        add_student_bmi = StudentBmi(
            ...
            height=int(get_students.height),
            weight=int(get_students.weight),
            hemoglobin=int(get_students.hemoglobin),
            age=get_students.age,
            month=get_students.month,
            bmiValue=int(get_students.bmiValue),
            studentId=ObjectId(get_students.id),
            createdOn=get_students.createdOn,
            status=2)
        updated_student_bmi_value=add_student_bmi.save()
        if updated_student_bmi_value:
            # flash("Successfully added Student BMI Status")
            # return redirect(url_for('studentsDetailsPage'))
